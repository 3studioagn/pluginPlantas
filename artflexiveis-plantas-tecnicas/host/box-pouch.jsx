// ==========================================
// BOX-POUCH.JSX — Box Pouch v2
// Portado 1:1 de reference/Box-Pouch_V2_0.jsx (desenharBoxPouch_Completo, linhas 440–978)
// Depende dos helpers definidos em core.jsx (mm2pt, cmyk, drawRect, drawLine,
// drawCotaH, drawCotaV, jsonOk, jsonErr, applyArialBold).
//
// Mudanças cirúrgicas em relação ao reference:
//   1. Declaração renomeada: desenharBoxPouch_Completo → gerarBoxPouch
//   2. Assinatura plana com flag hasZiper antes de ziperMM (padrão do painel —
//      espelha o checkbox "Adicionar Zíper Mickey" da UI; nomes alinhados ao
//      padrão do plugin: ziperAtivo→hasZiper, ziperDistMM→ziperMM).
...
//   6. addText e drawZiperMickey definidos como funções LOCAIS aninhadas
//      (shadow do global apenas dentro de gerarBoxPouch). Motivo: o reference
//      usa um addText com centralização aprimorada (top = y + h/2 com rot 90°)
//      diferente do core.jsx (top = y + h). Manter local preserva 1:1 a
//      posição dos labels SOLDA/FRENTE/VERSO/FUNDO sem afetar outras estruturas.
//      drawZiperMickey só existe nesta estrutura, então também fica local.
// Nada mais foi alterado (constantes, variáveis, ordem, agrupamentos, cores).
//
// V2 vs V1 (diferenças cobertas neste port):
//   • Faixa SUPERIOR (Sanfonas) ganhou um retângulo "Material" (cinza creme)
//     como filho direto do groupSanfonas, criado ANTES do subgrupo Sanfonas
//     — paridade visual e estrutural com a faixa inferior (Arte).
//   • Removidas as chamadas .zOrder(ZOrderMethod.SENDTOBACK) de matBot,
//     refileTopRect e refileBotRect: a ordem de criação já garante o
//     empilhamento correto (Material/Refile no fundo, blocos por cima).
// ==========================================

function gerarBoxPouch(compMM, largMM, sanfMM, hasZiper, ziperMM) {
    try {
        // Normaliza flag para booleano ExtendScript (ES3 — evalScript envia strings)
        hasZiper = (hasZiper === true || hasZiper === "true");

        // -----------------------------------------------------------------
        // HELPERS LOCAIS — específicos do Box Pouch (shadowing intencional
        // do addText global de core.jsx, e drawZiperMickey exclusivo desta
        // estrutura). Mantidos 1:1 com o reference.
        // -----------------------------------------------------------------
        ...
        // Validação
        if (utilFvLargMM <= 0) {
            return jsonErr("Sanfona aberta (" + sanfAbMM + " mm) >= largura (" + largMM + " mm). Útil frente/verso = " + utilFvLargMM + " mm.");
        }
        if (utilFvCompMM <= 0) {
            return jsonErr("Sanfona fechada (" + sanfMM + " mm) >= comprimento (" + compMM + " mm). Útil frente/verso = " + utilFvCompMM + " mm.");
        }
        ...
        if (hasZiper) {
            ...
            // Zíper sempre na FRENTE: borda direita = botB1M - ziperMM
            var zipXRight  = botB1M - mm2pt(ziperMM);
            ...
            drawCotaH(groupCotasBot, zipXRight, botB1M, yCotaZiper, fmt(ziperMM), corCota, 8);
